...
const OrdersList = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const response = await fetch(`${apiUrl}/neworders.php?${randomText}=${randomValue}`);
      if (!response.ok) {
  ...
  //   try {
  //     const response = await fetch(`${apiUrl}/cancelledOrder.php`, {
  //       method: "POST",
  //       headers: {
  ...
  //     // Refresh the page after cancelling the order
  //     //window.location.reload();
  //   } catch (error) {
  ...
  const handleClick = async (selectedOrder) => {
    try {
      ...
      const response = await fetch(`${apiUrl}/updateorder.php?${randomText}=${randomValue}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const printModalContent = () => {
    ...
    const formattedDate = currentDateUTC.toLocaleString("en-US", options);
        ...
        <html>
          ...
          <body>
            <div style="position: relative; text-align: center;">
              ...
              <div>Date: ${formattedDate.split(",")[0].trim()}</div>
              <div>Time: ${formattedDate.split(",")[1].trim()}</div>
            </div>
            </div>
          </body>
        </html>
  ...
  const handleClick_print = async () => {
    try {
      ...
      // Adjust date and time to CAT time zone (+02:00)
      const currentDateUTC = new Date();
      console.log("the current date: ", currentDateUTC);
      const options = { timeZone: "Africa/Lusaka", hour12: false };
      const formattedDate = currentDateUTC.toLocaleString("en-US", options);

      // Update the date and time in each order
      ordersToUpdate.forEach((order) => {
        order.order_date = formattedDate.split(",")[0].trim();
        order.order_time = formattedDate.split(",")[1].trim();
      });
	  
	  const randomValue = Math.floor(Math.random() * 1000); // Adjust the
      const randomText = generateRandomText(5);

      const response = await fetch(`${apiUrl}/updateorder_2.php?${randomText}=${randomValue}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
